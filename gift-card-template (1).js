...
function generateGiftCardHTML(data) {
  ...
  // Build sender div only if sender exists
  const senderDiv = giftSender ? '<div class="gift-sender">' + giftSender + '</div>' : '';
  
  // Format gift message
  const formattedMessage = (giftMessage || '').replace(/\n/g, '<br>');
...
'      <div class="gift-message">' + formattedMessage + '</div>' +
      senderDiv +
'    </div>' +
'  </div>' +
'</body>' +
