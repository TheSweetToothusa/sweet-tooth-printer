...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn-print { background: #4CAF50; color: white; }
    .btn-cancel { background: #e0e0e0; color: #333; text-decoration: none; display: inline-block; }
  </style>
</head>
<body>
  ...
  <form action="/dashboard/print-custom/${order.id}" method="POST">
    <div class="form-group">
      ...
      <input type="text" name="recipientName" value="${orderData.giftReceiver || orderData.recipient.name}" />
    </div>
    <div class="form-group">
      <label>Address Line 1</label>
      <input type="text" name="address1" value="${orderData.recipient.address1}" />
    </div>
    <div class="form-group">
      <label>Address Line 2 (City, State ZIP)</label>
      <input type="text" name="address2" value="${orderData.recipient.city}, ${orderData.recipient.province} ${orderData.recipient.zip}" />
    </div>
    <div class="form-group">
      <label>Gift Message</label>
      <textarea name="giftMessage">${orderData.giftMessage}</textarea>
    </div>
    <div class="form-group">
      <label>From (Sender)</label>
      <input type="text" name="giftSender" value="${orderData.giftSender}" />
    </div>
...
app.post('/dashboard/print-custom/:orderId', express.urlencoded({ extended: true }), async (req, res) => {
  try {
    const { recipientName, address1, address2, giftMessage, giftSender } = req.body;
